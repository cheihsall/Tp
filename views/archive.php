...
if (isset($_GET['lister']) && !empty($_GET['lister'])) {
  $currentPage = (int) strip_tags($_GET['lister']);
} else {
  $currentPage = 1;
}
$ins = $pdo->prepare("SELECT Matricule,Nom,Prenom,Email,DateArchiv FROM utilisateurs WHERE Etat=1");
$ins->execute();


$ins = $pdo->query("SELECT id,Nom,Prenom,Email,Matricule,DateArchiv,Etat FROM utilisateurs WHERE Etat=1 ");
...
$sql = 'SELECT COUNT(*) AS Etat FROM `utilisateurs` where Etat=1;';
...
$sql = 'SELECT * FROM `utilisateurs` ORDER BY `Prenom` DESC LIMIT :premier, :parpage; WHERE Etat=1';
...
if (isset($_POST["verif"])) {
  if (isset($_POST["classe"])) {
    ...
    if (!empty($classe)) {
      ...
      $list = "SELECT * FROM utilisateurs WHERE Etat=1 AND Nom LIKE '%$classe%' OR Prenom LIKE '%$classe%' ";
      $ins = $pdo->query($list);
    ...
    <td>
        ...
        <span  text-decoration: none;" OnClick="return(confirm(\'voulez-vous vraiment desarchiver cet employé?\'))";>
        <a href="../modeles/dsch.php?did=' . $ID . '" > <img style="height:40px ;width:40px;" src="image/dar.svg"></a></span>
        </td>
...
<nav style="position:absolute ; margin-top:-100px;" class="position-absolute top-100 start-25">
  <ul class="pagination">
    ...
    <li class="page-item <?= ($currentPage == 1) ? "disabled" : "" ?>">
      <a href="admin.php?lister=<?= $currentPage - 1 ?>" class="page-link">
        <</a>
    </li>
    <?php for ($page = 1; $page <= $pages; $page++) : ?>
      <!-- Lien vers chacune des pages (activé si on se trouve sur la page correspondante) -->
      <li class="page-item <?= ($currentPage == $page) ? "active" : "" ?>">
        <a href="admin.php?lister=<?= $page ?>" class="page-link"><?= $page ?></a>
      </li>
    <?php endfor ?>
    <!-- Lien vers la page suivante (désactivé si on se trouve sur la dernière page) -->
    <li class="page-item <?= ($currentPage == $pages) ? "disabled" : "" ?>">
      <a href="admin.php?lister=<?= $currentPage + 1 ?>" class="page-link">></a>
    </li>
  </ul>
</nav>
